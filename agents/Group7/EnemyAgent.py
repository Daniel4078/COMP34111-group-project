...
class EnemyAgent():
    ...
    def run(self, colour, board, turn):
    ...
    def _make_move(self):
        ...
        if (self._turn_count == 1):
            action = [int(math.sqrt(self._board_size)-1), int(math.sqrt(self._board_size)-1)]
            self.Swaped = False  
        else:
            newBoard = copy.deepcopy(self._board)
            value = self.MinMax(newBoard, True, self.depth, -math.inf, math.inf, [0,0])

            action = [self._best_move[0], self._best_move[1]]
